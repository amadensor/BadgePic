...
DEFAULT_UPDATE_RATE = 1000 # milliseconds
...
qr_payload_out = f"https://{CompanionWebAppURL}/digital_shadow.html?thing_name={thing_name}"

print("\n"*3)
print("Open this website on your laptop in a new browser tab:")
...
def handle_desired_shadow_state(desired_state):
    ...
    for k, v in desired_state.items():
        ...
        if k == 'display_brightness':
            ...
            badge.show_qr_code(v)
            payload['state']['reported']['picture'] = 'none'
            payload['state']['desired']['picture'] = None
        elif k == 'picture':
            badge.show_picture(v)
            payload['state']['reported']['qr_code'] = ''
            payload['state']['desired']['qr_code'] = None
